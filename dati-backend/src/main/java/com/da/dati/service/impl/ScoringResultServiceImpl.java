package com.da.dati.service.impl;
...
import com.da.dati.mapper.ScoringResultMapper;
import com.da.dati.model.dto.scoringResult.ScoringResultQueryRequest;
import com.da.dati.model.entity.App;
import com.da.dati.model.entity.ScoringResult;
import com.da.dati.model.entity.User;
import com.da.dati.model.vo.ScoringResultVO;
import com.da.dati.model.vo.UserVO;
import com.da.dati.service.AppService;
import com.da.dati.service.ScoringResultService;
import com.da.dati.service.UserService;
import com.da.dati.utils.SqlUtils;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (add) {
            // 补充校验规则
            ThrowUtils.throwIf(StringUtils.isBlank(resultName), ErrorCode.PARAMS_ERROR, "评分结果名称为空");
            ThrowUtils.throwIf(ObjectUtils.isEmpty(appId) || appId <= 0, ErrorCode.PARAMS_ERROR, "应用id为空");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (StringUtils.isNotBlank(resultName)) {
            ThrowUtils.throwIf(resultName.length() > 80, ErrorCode.PARAMS_ERROR, "评分结果名称过长");
        }

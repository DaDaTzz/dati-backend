...
public class UserAnswerServiceImpl extends ServiceImpl<UserAnswerMapper, UserAnswer> implements UserAnswerService {
    ...
    public void validUserAnswer(UserAnswer userAnswer, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(ObjectUtils.isEmpty(appId), ErrorCode.PARAMS_ERROR,"关联应用为空");
        }
        // 修改数据时，有参数则校验
        //  补充校验规则
        if (StringUtils.isNotBlank(choices)) {
            ThrowUtils.throwIf(choices.length() > 1024, ErrorCode.PARAMS_ERROR, "用户答案过长");
        }
    ...
    public QueryWrapper<UserAnswer> getQueryWrapper(UserAnswerQueryRequest userAnswerQueryRequest) {
        ...
        if (StringUtils.isNotBlank(searchText)) {
            // 需要拼接查询条件
            queryWrapper.and(qw -> qw.like("choices", searchText).or().like("resultName", resultName));
        }
        // 模糊查询
